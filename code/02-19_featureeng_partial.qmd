...
```{r}
##Pivot linger only to plot the and see the density.
...
```{r fe_month_sum}
##"Across" function to create a condition to calculate the mean and sum:
...
```{r fe_month_sum_wide}
##The "-" in select means select out from the data set.
##"paste0" means to paste together the information from 2 columns with a common separator as "_", and no space between them.
##Using the "across" function is to apply some function across multiple columns at the same time.
fe_month_sum_wide <- fe_month_sum %>%
  ...
  ##Using "~", you can call an function inside the function "across" without giving a name for it, then you can specify a condition in which the "." is related to each single column and the "1" is the number of decimal place. 
...
ggplot(data = fe_month_sum,
  ...
  scale_fill_viridis_c(option = "magma", alpha = .8) +
  theme(legend_positon = "none") +
  labs(x = "Monthly-avarage maximum temperature (C)",
       y = "Month"
       )
...
finalplots_all <- marrangeGrob(finalplots$plot,
                        ncol = 2, nrow = 3
                        )
